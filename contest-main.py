...
if __name__ == "__main__":
    # General vars
    STUDENT_NAME = 'Scarpanti.Soricaro'
    MY_CLF = None

    # Loading dataset
    x, y, feature_names, anomaly_perc = read_csv_binary_dataset(TRAIN_NAME)
    my_dataset = pandas.read_csv("./dataset_contest/UNSW_Students.csv")
    x = my_dataset.drop(columns=["is_ftp_login", "ct_ftp_cmd", "ct_flw_http_mthd", "multilabel"])
    ...
    MY_CLF = RandomForestClassifier(n_estimators=20)
    MY_CLF.fit(train_data, train_label)
